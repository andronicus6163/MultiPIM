...
def main():
    ...
    for lines in f:
        ...
        if len(l) > 2:
            val = float(l[2])
        # For non-graph types, use the actual value
    ...
    array = np.array([h, w, nnz, nnz], dtype=np.uint64)
    ...
    with open(f'{output_name}.baseline.bin', 'wb') as file:
