...
int test_exec(uint64_t num_ele){
  ...
  double* ref1 = new double[num_ele * num_ele];
  double* ref2 = new double[KERNEL_SIZE * KERNEL_SIZE];
  double* ref3 = new double[(num_ele - KERNEL_SIZE + 1) * (num_ele - KERNEL_SIZE + 1)];
  ...
  for(uint64_t i = 0; i < num_ele * num_ele; i++){
  ...
  for(uint64_t i = 0; i < num_ele - KERNEL_SIZE + 1; i++){
    ref3[i] = 0;
  }
  ...
  for (int i = 0; i < (num_ele - KERNEL_SIZE + 1) / OUTPUT_TILE_SIZE; i++) {
